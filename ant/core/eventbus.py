...
class EventBus(Worker):
    """Central event bus with subscription support and async dispathc"""
    ...
    def subscribe(self,
                  event_class: type[E], handler: Callable[[E], Awaitable[None]]
    ) -> None:
    ...
    def unsubscribe(self, handler: Handler) -> None:
        ...
        for event_class in self._subscribers:
            if handler in self._subscribers[event_class]:
                ...
                logger.debug(f"Unsbuscribed handler form {event_class.__name__} events")
    ...
    async def _notify_subscribers(self, event: Event) -> None:
        ...
        """gather 方法 所有匹配的处理函数是并发执行的，并且即使某个处理函数报错，
        也不会影响其他处理函数执行，错误会被收集并记录日志。"""
    ...
    async def _recover(self) -> int:
        """Recover pending events from previous crash. Returns count reoovered"""

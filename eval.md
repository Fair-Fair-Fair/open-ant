...
|---|---|---|
| baseline（无记忆） | 4.0%（4/100） | 6.0%（30/500） |
| memory（user-only 提取，生产口径） | **50.0%（50/100）** | — |
| memory（user+assistant 提取，对照） | 53.0%（53/100） | — |
| chunks（原始文本检索） | 67.0%（67/100） | **64.0%（320/500）** |
| oracle（evidence 注入） | 72.0%（72/100） | 72.6%（363/500） |

  子集行 = 从同一次全量 500 运行按 seed=42 过滤（同生成同 judge）；memory 两档仅 n=100。
